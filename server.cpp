...
bool send_all(int fd, const std::vector<std::uint8_t>& buf)
{
    ...
    while (rem)
    {
        ...
        if (n <= 0) return false;        // error or peer closed
        p   += n;
...
int main()
{
    ...
    for (;;)
    {
        ...
        while (true)
        {
            ...
            std::memcpy(packet.data() + sizeof(count_be),
                        floats.data(), floats.size() * sizeof(float));
            ...
            if (shift >  MAX_SHIFT || shift < -MAX_SHIFT) dir *= -1;
